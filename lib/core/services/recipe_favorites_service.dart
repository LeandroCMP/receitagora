...
class RecipeFavoritesService extends GetxService {
  ...
  void onInit() {
    super.onInit();
    _authSubscription = firebaseAuth.userChanges().listen(_handleUserChanged);
    _handleUserChanged(firebaseAuth.currentUser);
  }
  ...
  Future<void> addFavorite(RecipeEntity recipe) async {
    ...
    final document = _userFavoritesCollection(user.uid).doc(_buildRecipeId(recipe));
  ...
  User _requireAuthenticatedUser() {
    ...
    if (user == null) {
      throw FavoritesFailure('Faça login para salvar e gerenciar seus favoritos.');
    }
    return user;
  }
